...
SELECT prop.title, COUNT(prop.id) FROM (
	...
	AND ( (d.id IN (:DEPTID) OR d.parentid IN (:DEPTID) OR d.grandparentid IN (:DEPTID)) OR (i.value IN (SELECT xkonto FROM red19.neuro1) AND d2p2p.department_id = 1384) )
...
--AND ((d.id IN (1628,1627) OR d.parentid IN (1628,1627) OR d.grandparentid IN (1628,1627)) OR (i.value in (select xkonto from red19.neuro1) AND d2p2p.department_id = 1384) )


--1773 "Gillbergcentrum"
--1838 "Centrum för etik, juridik och mental hälsa"
--1872 "Centrum för åldrande och hälsa (AgeCap)"
